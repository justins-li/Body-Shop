...
Full layer-ownership table and rationale live in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md); endpoint reference in [docs/API.md](docs/API.md) — keep both updated when changing those surfaces. Planned direction (Tailwind/DaisyUI, ~180-exercise catalog, auth, Postgres, Vercel) is specified in [docs/ROADMAP.md](docs/ROADMAP.md); several items there deliberately reverse invariants below, so check it before assuming a constraint still holds.
