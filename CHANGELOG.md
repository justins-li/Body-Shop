...
- `flask --app app remap-exercises`, a one-off that moves entries logged against the
  four retired ids (`squat`, `bench_press`, `pull_ups`, `sit_ups`) onto their catalog
...
- **Exercise ids are now free-exercise-db's** (`Barbell_Squat`, `Sit-Up`) and are
  case-sensitive. The four hand-written ids are gone; run `remap-exercises` to carry an
